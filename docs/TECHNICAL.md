...
- **Consent:** Cookiebot CMP + Google Consent Mode v2, production-gated by
  `NEXT_PUBLIC_COOKIEBOT_ID` (no default). `lib/consent.ts` builds the GTM
  init script so `consent default` (denied) is pushed before `gtm.start`;
  Cookiebot `uc.js` (`data-blockingmode="none"` — GCM governs the tags, so
  auto-blocking could only interfere) fires `consent update`s. Same
  `/admin*` pathname gate keeps CMP out of admin documents. Full
  architecture and owner checklists: `docs/operations/analytics.md`.
...
| `NEXT_PUBLIC_COOKIEBOT_ID` | Cookiebot domain-group id (CMP + Consent Mode v2) | Production only — no default; unset → no CMP/consent scripts and behavior is exactly as without the integration |
